...
import random
from import3 import HumanMaleNames,HumanFemaleNames, HumanLastNames
...
def getRandomFirstName():
    ...
    if getRandomSex() == name:
        random.choice(HumanMaleNames)
    else:
        random.choice(HumanFemaleNames)
    return FirstName


def getRandomeLastName():
    #Modify me so I return something from your text files.
    random.choice(HumanLastNames)
    return FirstName
